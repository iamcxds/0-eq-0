...
\begin{frame}{Fibration de Hopf}
  ...
  Pour tout point $x\in \mathbb{S}^2$, l’image réciproque $H^{-1}(x)$ est un cercle $\mathbb{S}^1$. 
\pause
...
\begin{frame}{Espace des ensembles finis}
  Au lieu de considérer seulement l’ensemble des ensembles finis, nous considérons maintenant l’espace $F = \mathrm{FinSet}^{\cong}$ : chaque point $x\in F$ correspond à un ensemble fini, et un chemin entre deux points correspond à un isomorphisme $f:x\xrightarrow{\cong} y$ entre les ensembles finis. 
  \pause
...
\begin{frame}{K-groupes des ensembles finis}
  Posons $K_i(\mathrm{FinSet})=\pi_i K(F)$ pour les K-groupes supérieurs. On a alors les observations suivantes :
  \pause
  \begin{itemize}
    \item $\pi_0 K(F) = \mathbb{Z}$, by $ x \mapsto \|x\|$. This coincides with $K_0(\mathrm{FinSet})$. 
      \pause
    \item On peut construire un lacet $h\in \pi_1 K(F)$ en utilisant les identifications 
      \[
        h: \emptyset \sim \{+,-\} \sim \{-,+\} \sim \emptyset
      \]
      \pause
    \item En fait, $\pi_1 K(F)=\mathbb{Z}/2$, et $h$ en est un générateur. Autrement dit, $h$ donne une preuve non triviale de $0=0$. 
  \end{itemize}
  \pause
  \begin{block}{Réponse}
...
\begin{frame}{K-groupes algébriques d’un corps}
  Soit $k$ un corps. On peut effectuer la même construction pour les espaces vectoriels $k$-linéaires de dimension finie. Si l’on note $V_k=\mathrm{FinVect}_k^{\cong}$, on définit les \textbf{K-groupes algébriques} de $k$ par $K_i(k)=\pi_i K(V_k)$.
...
\begin{frame}{Et plus encore ?}
  ...
  \pause
\item Ainsi, $K(F)$ peut aussi être vu comme $\mathrm{Conf}^{fr}(\mathbb{R}^{\infty})$, et l’application de Hopf $H$ comme le lacet $h$ peuvent tous deux y être représentés par des mouvements.
  \end{itemize}
  \pause
...
\only<1-2>{Prenons comme exemple $\Omega^{V_k,\oplus}_n$, où $V_k=\mathrm{FinVect}^{\cong}_k$ est la sous-catégorie des espaces vectoriels de dimension finie dont les morphismes sont tous des isomorphismes.} Dans ce cas, on peut voir une lasagne comme un treefold/une variété muni(e) d’un système local en $k$.
  \pause
\begin{itemize}
...
\end{center}
\pause
Remarquons que $k^{\times}\cong \mathrm{GL}_{\infty}(k)/[\mathrm{GL}_{\infty}(k),\mathrm{GL}_{\infty}(k)]$. En fait, on peut montrer que le morphisme induit $L:k^{\times} \xrightarrow{\cong} \Omega^{V_k,\oplus}_1$ est un isomorphisme. La lasagne induite par l’application de Hopf $H$ est alors donnée par $L(-1)$.
...
\end{tikzpicture}
 ...
 \pause
Rappelons que $K_2(k)= k^\times \otimes k^\times / \langle a \otimes 1-a \rangle$, ce qui signifie qu'il nous faut trouver une $3$-lasagne appelée lasagne de Ghys qui bornes $C(a,1-a)$. Autrement dit, on peut encoder une preuve/un calcul par une lasagne.
...
\end{frame}
\begin{frame}{Une $3$-lasagne borde $C(e_{12}(a),e_{12}(b))$}
