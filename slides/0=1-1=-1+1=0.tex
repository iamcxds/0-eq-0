...
\begin{frame}{Algebraic K-groups of a field}
  Let $k$ be a field, we can follow the same construction for finite dimensional $k$ vector space. Let $V_k=\mathrm{FinVect}_k^{\cong}$, we define \textbf{Algebraic K-groups} of $k$ as $ K_i(k)= \pi_i K(V_k)$.

(Notice that we can think finite sets as $\mathbb{F}_1$ vector spaces)
...
\end{tikzpicture}
  ...
  \begin{block}{New Question}
    Can we find geometric interpretation for general K-groups $K_i(k)$ ?  
 \end{block} 
\end{frame}

\begin{frame}{What about cobordism?}

If we put the movement in 3D space, we get a circle:
\begin{columns}
...
\begin{frame}{Intrinsic Skein-Lasagna}
  A $n$-\textbf{treefold} is a space generally locally homeomorphic to $T\times \mathbb{R}^{n-1}$, where $T$ is a tree. \pause
  Give a symmetric monoidal category $(\mathcal{C},\otimes)$, define an $n$-$(\mathcal{C},\otimes)$-\textbf{lasagna} $L$ as a framed $n$-treeifold $\mathcal{T}$ with a stratification $ \mathcal{S}$ and the strata are marked compatibly with category $\mathcal{C}$:
  \pause
  \begin{itemize}
    \item $0$-codimensional strata are marked with objects; For $i>0$, $i$-codimensional strata are marked with $i$-morphism,
    \item At the branches of treefold marked with the monoidal product $\otimes$.
  \end{itemize}
...
\end{tikzpicture}
  ...
  \begin{block}{Relation with "Skein-lasagna module"}
    ...
    We do not ask for an ambient space $X$ where lasagna embeds (or think $X=\mathbb{R}^\infty$). This means we study the intrinsic geometry of lasagna itself.
 \end{block} 
 \pause

    We can think a lasagna $L$ as a string diagram of $\mathcal{C}$ drawing on $ \mathcal{T}$, or another way to say $L\in \int_{\mathcal{T}} (\mathcal{C},\otimes)$.

    \begin{block}{Cobordism of lasagna}
    
    As one expects, we can define the cobordism relation between $L_1,L_2$, if there is a lasagna $W$ with boundary such that $\partial W= L_1 \sqcup -L_2 $. Finally, we define the $n$-th $(\mathcal{C},\otimes)$-\textbf{cobordism group} $\Omega^{\mathcal{C},\otimes}_n$ to be the abelian group generated by $n$-lasagna modulo cobordism relations.

  \end{block}

\end{frame}

\begin{frame}{ $(V_k,\oplus)$-Cobodrism Group}
  As an example $\Omega^{V_k,\oplus}_n$, where $V_k=\mathrm{FinVect}^{\cong}_k$ is the subcategory of finite-dimensional vector space where the morphisms are all isomorphisms. In this case, we can think a lasagna as a treefold /manifold equips with a $k$-local system.
  \pause
\begin{itemize}
  \item It is clear $ \Omega^{V_k,\oplus}_0\cong \mathbb{Z}$ is classfied dimension.
  \pause

\item  $ \Omega^{V_k,\oplus}_1$ is generated by circle $L_V(f)$ marked with vector spaces $V$ and matrix $ f\in \mathrm{GL}_V(k)$. And by adding identity matrices, we find we have a homomorphism $L:\mathrm{GL}_{\infty}(k) \twoheadrightarrow \Omega^{V_k,\oplus}_1$.
...
\begin{frame}
 We have one more relation given by surfaces: 
\begin{center}
...
\end{center}
\pause
Notice that $k^{\times}\cong \mathrm{GL}_{\infty}(k)/[\mathrm{GL}_{\infty}(k),\mathrm{GL}_{\infty}(k)]$. In fact, one can show the induced homomorphism $ L:k^{\times}  \xrightarrow{\cong} \Omega^{V_k,\oplus}_1$ is isomorphism. And the lasagna induced by Hopf map $H$ is given by $L(-1)$.

\end{frame}
\begin{frame}{Generalized Pontryagin–Thom Isomorphism}
  
Recall that $K_0(k)=\mathbb{Z},K_1(k)=k^{\times}$. This is not just a coincidence:
\pause
\begin{block}{Generalized Pontryagin–Thom Isomorphism (My claim)}
...
\end{block}
\pause
This answers the new question, which indicates we can view lasagna as elements in $K$-groups, which provides a bridge from hard problems in algebra toward hard problems in geometric topology.

So what does it tell us about $n=2$ ?

\end{frame}
\begin{frame}{$\Omega^{V_k,\oplus}_2$ and $K_2(k)$}

  For matrices $f,g\in \mathrm{GL}(k)$ such that $[f,g]=1$, one can define following $2$-lasagna $C(f,g)$:
...
\end{tikzpicture}
 ...
 \pause
 Recall $K_2(k)= k^\times \otimes k^\times / \langle a \otimes 1-a \rangle$, which means we should find a $3$-lasagna called Ghys' lasagna who bounds $C(a,1-a)$. In other word, we can encode proof/computation by a lasagna.
\pause 

I don't have Ghys' lasagna for now (which has Heegaard genus less than 26), but I have an easier example.
\end{frame}
\begin{frame}{$3$-Lasagna bounds $C(e_{12}(a),e_{12}(b))$}
  For $i\neq j$, let elementary matrices $e_{ij}(a)$ to be
  \[
  ...
  \]
  \pause
  Notice $ e_{ij}(a)^{-1}=e_{ij}(-a)$, and they satisfy the Steinberg relations:
  \[
  ...
  \]
  \pause
  Using these relations and Hall-Witt identity, we have the following non-trivial proof of $[e_{12}(a),e_{12}(b)]=\mathbf{1}$:
...
\end{frame}
\begin{frame}
  Following this blueprint, we have such Heegaard splitting to construct a lasagna: 
\begin{center}
...
% Text Node
\draw (278.17,328.4) node [anchor=north west][inner sep=0.75pt]  [font=\LARGE]  {$=\partial $};
% Text Node
