...
\begin{frame}{Algebraic K-groups of a field}
  Let $k$ be a field. We can carry out the same construction for finite-dimensional $k$-vector spaces. Let $V_k=\mathrm{FinVect}_k^{\cong}$, and define the \textbf{algebraic K-groups} of $k$ by $K_i(k)=\pi_i K(V_k)$.
...
\end{center}
\pause
Notice that $k^{\times}\cong \mathrm{GL}_{\infty}(k)/[\mathrm{GL}_{\infty}(k),\mathrm{GL}_{\infty}(k)]$. In fact, one can show that the induced homomorphism $L:k^{\times} \xrightarrow{\cong} \Omega^{V_k,\oplus}_1$ is an isomorphism. The lasagna induced by the Hopf map $H$ is then given by $L(-1)$.
