...
\begin{frame}{ $(V_k,\oplus)$-Cobodrism Group}
  As an example $\Omega^{V_k,\oplus}_n$, where $V_k=\mathrm{FinVect}^{\cong}_k$ is the subcategory of finite-dimensional vector space where the morphisms are all isomorphisms.
  \pause
\begin{itemize}
...
\end{tikzpicture}
 ...
 Recall $K_2(k)= k^\times \otimes k^\times / \langle a \otimes 1-a \rangle$, which means we can find a $3$-lasagna who bounds $C(a,1-a)$. In other word, we can encode prove/computation by a lasagna.
 
I don't have a lasagna bounds $C(a,1-a)$ for now, but I have an easier example.
\end{frame}
\begin{frame}{$3$-Lasagna bounds $C(e_{12}(a),e_{12}(b))$}
  ...
  \[
    ...
    \end{cases}
  \]
  Using these relations and Hall-Witt identity, we have the following non-trivial prove of $[e_{12}(a),e_{12}(b)]=\mathbf{1}$:
...
\end{frame}
\begin{frame}
  Following this blueprint, we have such handle decomposition to construct a lasagna: 
\begin{center}
