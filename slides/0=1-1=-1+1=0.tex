...
\begin{itemize}
  \item For $i<n$, $\pi_i(\mathbb{S}^n)=\{ 0 \}$, where $0: \mathbb{S}^i\to * \xrightarrow{\mathrm{base}} \mathbb{S}^n$.
    \pause
  \item For $\pi_n(\mathbb{S}^n)$, we consider the space $\mathrm{End}_*(\mathbb{S}^n)=\mathrm{Map}_*(\mathbb{S}^n,\mathbb{S}^n)$.
\end{itemize}
\pause
...
\[ \mathrm{End}_*(\mathbb{S}^n) \cong \mathrm{Conf}( \mathbb{R}^n) = \bigcup_{j,k}\mathrm{Emb}( \bigsqcup_j D^n_+ \sqcup \bigsqcup_k D^n_- , \mathbb{R}^n )
\]
...
\begin{itemize}
  ...
  \item $\pi_i(F,x) = \{0\}$ for $i>0$ (one can never deform an isomorphism of set to another). 
...
\begin{frame}{What's more?}
  ...
  \pause
  \begin{itemize}
    \item In fact, $K_i(\mathrm{FinSet}) \cong \lim_{n\to \infty}\pi_{i+n}(\mathbb{S}^n)$.
  \pause
    \item So, $K(F)$ can be seen as $\mathrm{Conf}( \mathbb{R}^{\infty})$, and both the Hopf map $H$ and $h$ can be represented as movements within it.
  \end{itemize}
  \pause
