...
\subtitle{From Elementary Arithmetic to \\ Intrinsic Skein-Lasagna}
\author{Keyao Peng}
...
But what exactly is an \textbf{equation}? 

\pause
The statement $A=B$ says that $A$ \textbf{is} equal to $B$, but \textbf{how} they are equal depends on the \textit{proof}.
...
\pause
  We can visualize this map by identifying $\mathbb{R}^3\cong \mathbb{S}^3-\{\infty\}$, so that the Hopf fibration appears as a map from $\mathbb{R}^3$ to $\mathbb{S}^2$:
\begin{figure}
  \begin{center}
...
\begin{itemize}
  \item $\pi_0 X \in \mathrm{Set} $, connected component, how many "pieces" $X$ has. 
    \pause
  \item Choose a base point $x\in X$. Let $\mathrm{Map}_*(\mathbb{S}^1,X)\in \mathrm{Space}$ be the \textbf{space} of loops in $X$ based at $x$. Here $\mathrm{Map}_*$ is the \textbf{space} of pointed continuous maps, equipped with the topology coming from continuous deformation. 
  \only<3>{
...
\end{tikzpicture}
    ...
    \pause
  \item $\pi_1(X,x) := \pi_0\mathrm{Map}_*(\mathbb{S}^1,X) \in \mathrm{Group}$ classifies loops in $X$ up to deformation. This is the fundamental group.
    \pause
  \item For $i>1$,
    \[
      \pi_i(X,x) := \pi_0\mathrm{Map}_*(\mathbb{S}^i,X)\cong \pi_0\mathrm{Map}_*(\mathbb{S}^1,\mathrm{Map}_*(\mathbb{S}^{i-1},X)) \in \mathrm{Abel}.
    \]
     This is the $i$th homotopy group.
\end{itemize}
\pause
...
\begin{itemize}
  \item For $i<n$, $\pi_i(\mathbb{S}^n)=\{ 0 \}$, where $0: \mathbb{S}^i\to * \xrightarrow{\mathrm{base}} \mathbb{S}^n$.
    \pause
  \item To study $\pi_n(\mathbb{S}^n)$, let us consider the space $\mathrm{End}_*(\mathbb{S}^n):=\mathrm{Map}_*(\mathbb{S}^n,\mathbb{S}^n)$.
\end{itemize}
\pause

Let $f\in \mathrm{End}_*(\mathbb{S}^n)$, and choose a small disk $D^n_{\epsilon}\subset \mathbb{S}^n$ away from the base point. Then, for $\epsilon$ sufficiently small, the preimage $f^{-1}(D^n_{\epsilon})\subset \mathbb{S}^n-\{*\}\cong \mathbb{R}^n$ is a disjoint union of disks. Notice that the disks in this union have orientations $+$ and $-$.
...
\]
Then $H$ corresponds to a path (that is, a homotopy) from $0\in \mathrm{End}_*(\mathbb{S}^2)$ back to itself.
...
\begin{frame}{Visualize $H$ as deformation}
  ...
  \end{figure}
  \pause
  We can also visualize it as a movement in the configuration space. We will show it later.
...
\begin{frame}{How do we define $\mathbb{Z}$?}
\pause
We can define the natural numbers $\mathbb{N}$ as the isomorphism classes of finite sets.
\[
...
\begin{frame}{Space of finite sets}
  Instead of thinking the set of "finite sets", we now consider the \textbf{(moduli) space} of them $F = \mathrm{FinSet}^{\cong}$ : 
  \begin{itemize}
    \item every point $x\in F$ corresponds to a finite set,
    \item a path between to points corresponds to an isomorphism $f:x\xrightarrow{\cong} y$ of two finite sets.
  \end{itemize}
  \pause
...
\begin{frame}{K-space (spectrum) of finite sets}
  We now want to add “negative” elements to $F$. To do this, we consider the space $K(F)$ of \textbf{ordered finite sets} whose elements are marked by $+$ or $-$. We identify signed sets with the same value $\|X\| = |X_+|-|X_-|$, and these identifications are represented by paths.
\pause
\begin{center}
...
\begin{frame}{K-groups of finite sets}
  Let $ K_i(\mathrm{FinSet})= \pi_i (K(F),\emptyset)$ to be the higher K-groups, we have the following observations:
  \pause
  \begin{itemize}
    \item $\pi_0 K(F) = \mathbb{Z}$, by $ x \mapsto \|x\|$. This coincides with $K_0(\mathrm{FinSet})$. 
      \pause
    \item We can construct a loop $h \in \pi_1 (K(F),\emptyset) $ by using identifications 
      \[
        h: \emptyset \sim \{+,-\} \sim \{-,+\} \sim \emptyset
      \]
      \pause
    \item In fact $\pi_1 (K(F),\emptyset)= \mathrm{S}_n^{ab}=\mathbb{Z}/2 $ with $h$ as the generator. That is to say, $h$ is a non-trivial identification. 
  \end{itemize}
  \pause
  \begin{block}{Answer}
...
\begin{frame}{Algebraic K-groups of a field}
  Let $k$ be a field. We can carry out the same construction for finite-dimensional $k$-vector spaces. Let $V_k=\mathrm{FinVect}_k^{\cong}$, and define the \textbf{algebraic K-groups} of $k$ by $K_i(k)=\pi_i (K(V_k),\mathbf{0})$.

(You may think of finite sets as vector spaces over $\mathbb{F}_1$.)

\pause
Algebraic K-groups are important invariants with connections to geometry, topology, ring theory, and number theory, but are notoriously difficult to compute.
\pause
\begin{itemize}
  \item $K_0(k)=\mathbb{Z}$
  \item $K_1(k)=k^{\times}$
...
\pause
\column{0.5\linewidth}
This gives a framed circle, that is, an embedding $\mathbb{S}^1 \hookrightarrow \mathbb{R}^n$ together with a \textbf{trivialization} $\tau$ of the normal bundle. We write $\Omega^{fr}_i$ for the group of cobordism classes of $i$-dimensional framed manifolds. \pause We also have:

\[
  \Omega^{fr}_i \cong \pi^s_i(\mathbb{S}) 
\]

Therefore, this framed circle is nontrivial and generates $\Omega^{fr}_1$.

\end{columns}
\end{frame}
\section{Lasagna}
\begin{frame}{Intrinsic Skein-Lasagna}
  An $n$-\textbf{treefold} is a space that is locally homeomorphic to $T\times \mathbb{R}^{n-1}$, where $T$ is a tree. \pause
  Given a symmetric monoidal category $(\mathcal{C},\otimes)$, define an $n$-$(\mathcal{C},\otimes)$-\textbf{lasagna} $L$ to be a framed $n$-treefold $\mathcal{T}$ with a stratification $\mathcal{S}$, whose strata are labeled compatibly with $\mathcal{C}$:
  \pause
  \begin{itemize}
    \item $0$-codimensional strata are labeled by objects; for $i>0$, $i$-codimensional strata are labeled by $i$-morphisms,
    \item At the branches of the treefold, we mark how pieces are combined using the monoidal product $\otimes$.
  \end{itemize}
...
\end{tikzpicture}
  ...
  \begin{block}{Relation with "Skein-lasagna module"}
 ...
 \pause
    ...
    We can think of a lasagna $L$ as a string diagram for $\mathcal{C}$ drawn on $\mathcal{T}$; equivalently, one may write $L\in \int_{\mathcal{T}} (\mathcal{C},\otimes)$.
\pause
    \begin{block}{Cobordism of lasagna}
    
    As expected, we define a cobordism relation between $L_1$ and $L_2$ if there exists a lasagna $W$ with boundary such that $\partial W = L_1 \sqcup -L_2$. Finally, we define the $n$th $(\mathcal{C},\otimes)$-\textbf{cobordism group} $\Omega^{\mathcal{C},\otimes}_n$ to be the abelian group generated by $n$-lasagnas modulo these cobordism relations.

  \end{block}

\end{frame}

\begin{frame}{$(V_k,\oplus)$-Cobordism Group}
  As an example, consider $\Omega^{V_k,\oplus}_n$, where $V_k=\mathrm{FinVect}^{\cong}_k$ is the category of finite-dimensional vector spaces with all isomorphisms as morphisms. In this case, one can think of a lasagna as a treefold/manifold equipped with a $k$-\textbf{local system}.
  \pause
\begin{itemize}
  \item Clearly, $\Omega^{V_k,\oplus}_0\cong \mathbb{Z}$, classified by dimension of vector space.
  \pause

\item $\Omega^{V_k,\oplus}_1$ is generated by circles $L_V(f)$ labeled by a vector space $V$ and a matrix $f\in \mathrm{GL}_V(k)$. By stabilizing with identity matrices, we obtain a homomorphism $L:\mathrm{GL}_{\infty}(k) \twoheadrightarrow \Omega^{V_k,\oplus}_1$.
...
\end{center}
\pause
Notice that $k^{\times}\cong \mathrm{GL}_{\infty}(k)/[\mathrm{GL}_{\infty}(k),\mathrm{GL}_{\infty}(k)]$. In fact, one can show that the induced homomorphism $L:k^{\times} \xrightarrow{\cong} \Omega^{V_k,\oplus}_1$ is an isomorphism. 

\pause
For example, the lasagna induced by the Hopf map $H$ is then given by $L(-1)$.

\end{frame}
\begin{frame}{Generalized Pontryagin–Thom Isomorphism}
  
Recall that $K_0(k)=\mathbb{Z}$ and $K_1(k)=k^{\times}$. This is not just a coincidence:
\pause
\begin{block}{Generalized Pontryagin--Thom Isomorphism (Claim)}
...
\end{frame}
\begin{frame}{$\Omega^{V_k,\oplus}_2$ and $K_2(k)$}
...
\end{tikzpicture}
 ...
 \pause
 Recall that $K_2(k)= k^\times \otimes k^\times / \langle a \otimes (1-a) \rangle$, which means that we should find a $3$-lasagna, called Ghys' lasagna, that bounds $C(a,1-a)$. In other words, we can encode a proof or computation by a lasagna.
\pause 

I do not yet have Ghys' lasagna in hand (it has Heegaard genus less than $26$), but I do have an easier example.
\end{frame}
\begin{frame}{$3$-Lasagna bounds $C(e_{12}(a),e_{12}(b))$}
...
\]}
  \pause
  Notice that $e_{ij}(a)^{-1}=e_{ij}(-a)$, and these matrices satisfy the Steinberg relations:
  \[
  ...
  \]
  \pause
  Using these relations and the Hall--Witt identity, we obtain the following nontrivial proof that $[e_{12}(a),e_{12}(b)]=\mathbf{1}$:
  {\begin{eqnarray*}
...
\end{tikzpicture}
  \end{center}
\end{frame}
\begin{frame}{Thank you! I hope you enjoyed and gained some new insights.}
